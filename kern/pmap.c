...
void page_init(void) {
	...
	u_long usepage = PPN(PADDR(freemem));
	for (u_long i = 0; i < usepage; i++){
		pages[i].pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for (u_long i = usepage; i < npage; i++){
		pages[i].pp_ref = 0;
 ...
 *   wheeher this function succeeds or not.
 */
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	else if(!(*pgdir_entryp & PTE_V) && !create){
		*ppte = NULL;
		return 0;
	}
